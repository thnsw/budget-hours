...
class PDFReportGenerator:
    ...
    def create_pdf_report(self, classified_data: List[Dict[str, Any]], output_path: Optional[str] = None) -> str:
        ...
        # Add overall statistics section on its own page
        pdf.add_page()
        pdf.set_font("Times", "B", 16)
        pdf.cell(0, 10, "Overall Statistics", ln=True)
        pdf.outline.add_section("Overall Statistics")
        ...
        # Add Customer Statistics section on a new page
        pdf.add_page()
        pdf.set_font("Times", "B", 16)
        pdf.cell(0, 10, "Customer Statistics", ln=True)
        pdf.outline.add_section("Customer Statistics")
        pdf.set_font("Times", "", 10)
        ...
        # Process customers and their projects
        for customer_name, customer in customer_data.items():
            # Add customer overview section
            pdf.add_page()
            pdf.set_font("Times", "B", 16)
            customer_title = f"Customer Overview: {customer_name}"
            pdf.cell(0, 10, customer_title, ln=True)
            # Add this section to outline/TOC
            customer_section = pdf.outline.add_section(customer_title)
            ...
            # Now add detailed project info for each of this customer's projects
            customer_projects = [p for p in project_data.keys() 
                                if any(entry.get('customer_name') == customer_name and 
                                       entry.get('project_name') == p 
                                       for entry in classified_data)]
            ...
            for project in customer_projects:
                if project in projects:
                    ...
                    project_title = f"Project: {project}"
                    pdf.cell(0, 10, project_title, ln=True)
                    # Add this project as a subsection to the customer
                    pdf.outline.add_section(project_title, parent=customer_section)
